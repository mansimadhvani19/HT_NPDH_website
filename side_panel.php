<div class="side-panel hide">
    <div class="sp-box">
    <div class="sp-close">
    <span class="close-sp"><i class="fa fa-times-circle" aria-hidden="true"></i></span>
    </div>
    <div class="sp-logo">
    <a href="index.php">
    <img src="assets/Final_images/100X100 (2).png" alt>
    </a>
    </div>
    <div class="sp-text">
    <p>We are committed to delivering exceptional events of all kinds, tailored to your vision and needs. Let us take care of every detail, so you can enjoy the celebration to the fullest.</p>
    </div>
    <div class="sp-recent-post">
    <h4>Recent Post</h4>
    <div class="sp-single-rp">
    <div class="ssrp-img">
    <img src="assets\recent-blog3.jpg" alt>
    </div>
    <div class="ssrp-text">
    <a href="">Flower-Adorned Welcome: Where Love Takes Root</a>
    <span class="date-meta"><i class="fa fa-calendar" aria-hidden="true"></i>01 Dec 2020 </span>
    </div>
    </div>
    <div class="sp-single-rp">
    <div class="ssrp-img">
    <img src="assets\recent-blog1.jpg" alt>
    </div>
    <div class="ssrp-text">
    <a href="">Endless Vows, Timeless Glamour: Our Decorative Affair </a>
    <span class="date-meta"><i class="fa fa-calendar" aria-hidden="true"></i>25 Nov 2020 </span>
    </div>
    </div>
    <div class="sp-single-rp">
    <div class="ssrp-img">
    <img src="assets\recent-blog2.jpg" alt>
    </div>
    <div class="ssrp-text">
    <a href="">Cherishing Love, Making Memories</a>
    <span class="date-meta"><i class="fa fa-calendar" aria-hidden="true"></i>30 Nov 2020 </span>
    </div>
    </div>
    </div>
    <div class="sp-contact">
    <h4>Contact</h4>
    <div class="spc-text">
    <p>M-87/1035 Panchvati Apartment, Sola Road, Naranpura, Ahmedabadd</p>
    <h5>Phone:</h5>
    <p>+91 8511055545</p>
    <h5>Email:</h5>
    <p><a href="https://demo.voidcoders.com/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="e98c84888085a98c91888499858cc78a8684">npdhevents1980@gmail.com</a></p>
    </div>
    </div>
    <div class="sp-social">
    <ul class="social-list">
    <!-- <li data-aos="fade-up" data-aos-duration="1500"><a href><i class="flaticon-twitter"></i></a></li> -->
    <li data-aos="fade-up" data-aos-duration="2000"><a href><i class="flaticon-facebook"></i></a>
    </li>
    <li data-aos="fade-up" data-aos-duration="2500"><a href = "https://www.instagram.com/npdhevents_1980/"><i class="flaticon-instagram"></i></a>
    </li>
    <li data-aos="fade-up" data-aos-duration="3000"><a href = "https://www.linkedin.com/feed/"><i class="flaticon-linkedin"></i></a>
    </li>
    </ul>
    </div>
    </div>
    </div>